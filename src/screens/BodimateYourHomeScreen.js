...
import {bodimateYourHomeValidate} from "../utils/validation";
...
function BodimateYourHomeScreen({navigation}) {
    ...
    const {
        ...
        setValue
    } = FormHandler(register, bodimateYourHomeValidate);
    ...
    const openBottomSheet = (imageType) => {
    ...
    const pickImage = () => {
        launchImageLibrary({mediaType: "photo"}, (response) => {
            if (response.assets && response.assets.length > 0) {
                ...
                if (currentImageType === "profilePic") {
                    setProfileImage(selectedImageUri);
                    setValue({profilePic: selectedImageUri});
                } else if (currentImageType === "nicFront") {
                    setNicFront(selectedImageUri);
                    setValue({nicFront: selectedImageUri});
                } else if (currentImageType === "nicBack") {
                    setNicBack(selectedImageUri);
                    setValue({nicBack: selectedImageUri});
                }
            }
            closeBottomSheet();
        });
    ...
    return (
        <ScrollView contentContainerStyle={{flexGrow: 1}}>
            <View style={globalStyles.container}>
                ...
                <TouchableOpacity onPress={() => openBottomSheet("profilePic")} style={styles.profileImageContainer}>
                    {isUploading ? (
                ...
                <Text style={[globalStyles.textLabel, {marginTop: 20}]}>NIC Front</Text>
                <TouchableOpacity onPress={() => openBottomSheet("nicFront")} style={styles.selectImageButton}>
                    <Text style={styles.buttonText}>Select Image</Text>
                </TouchableOpacity>
                {nicFront && (
                    <Image source={{uri: nicFront}} style={styles.selectedImagePreview}/>
                )}

                <Text style={[globalStyles.textLabel, {marginTop: 20}]}>NIC Back</Text>
                <TouchableOpacity onPress={() => openBottomSheet("nicBack")} style={styles.selectImageButton}>
                    <Text style={styles.buttonText}>Select Image</Text>
                </TouchableOpacity>
                {nicBack && (
                    <Image source={{uri: nicBack}} style={styles.selectedImagePreview}/>
                )}
                    ...
                    <View style={styles.modalOverlay}>
                        <TouchableWithoutFeedback>
                            ...
                            >
                                ...
                                {currentImageType === "profilePic" && (<View style={styles.profileImageContainerLarge}>
                                    {profileImage ? (
...
export default BodimateYourHomeScreen;
